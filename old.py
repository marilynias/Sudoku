...
def getSudokus(num: int, csv_file):
    if not csv_file:
        ...
        logging.info(
            f"generating {num} Sudokus ... this might take a while (~{round(num/7, 2)}s)")
        logging.info(
            "if you feed me a csv with sudokus, you can skip this step")
...
def getArgs() -> argparse.Namespace:
    ...
    parser.add_argument('-f', '--file', type=str, default="sudoku17.csv",
...
class SudokuGrid():
    ...
    def get_connected_values_from_sud(self, index: int, sudoku: List[int]):
        return [sudoku[i] for i in self.get_connected_set_to_ind(index)]
